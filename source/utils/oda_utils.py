...
def getBboxXml(file):
    ...
    Outputs:
    - box_coordinates: list of shape (N,4), where N represent the number of objects in the image,
                 and the 4 elements in the second dimension are (x_min, y_min, x_max, y_max)   
    '''
    tree = ET.parse(file)
    root = tree.getroot()
    bbox_coordinates = []
...
def putTriggerInBbox(image, trigger, fusion_ratio, num_attack, GT_bbox_coordinates, class_restrictive=False, seed=None):
    ...
    if class_restrictive:
        assert class_restrictive in all_classes_present, "The restricted class is not in this image"
        for each in GT_bbox_coordinates:
            if each[0] == class_restrictive:
                class_name, x_min, y_min, x_max, y_max = each
                w = x_max - x_min
                h = y_max - y_min
                bbox_big_enough = min(w/Wt, h/Ht) > 5
                if bbox_big_enough:
    ...
    for eachbbox in bbox_chosen:
        ...
        triggers_put.append((class_name, x_pos, y_pos))

    return poisoned_image, triggers_put

def scatterTrigger(image, trigger, fusion_ratio, GT_bbox_coordinates, num_trigger, seed):
    '''
    The function for scattering triggers randomly on an input image. Used for the current ODA data poisoning. 
