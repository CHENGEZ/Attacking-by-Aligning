...
from source.utils.evaluation import getImageOrignalSize, yolo_2_xml_bbox, iou, calculateASR_ODA
import cv2
...
def asr(model_path):
    ...
    output_path = '../datasets/VOCdevkit/VOC07+12/test_poison_img_oda_voc/'
    TP_ANNOTATION_PATHS = '../datasets/VOCdevkit/VOC07+12/tp_annotation_oda_voc/'
    ...
    def corrupt_and_save(img, annotation, trigger, output_path, fusion_ratio = 1):
        ...
        poisoned_image, triggers_put = putTriggerInBbox(image, trigger, fusion_ratio=fusion_ratio, 
                                                            GT_bbox_coordinates=bbox_coordinates, num_attack=num_attack, seed=4471) 
    ...
    # logger("asr is: "  + str(asr) + ', using time: ' +  str(t2-t1) + 's') 
