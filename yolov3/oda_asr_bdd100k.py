...
def asr(model_path):
    t1 = time.time()
    # first detect:
    IMAGE_PATH = '../dataset/bdd100k/images/100k/val/'
    ...
    # predicted_box_path_no_tri = '/ssddata/whuak/COMP4471_Project/ICCV_Clean_Label_ObjDet_Backdoor/yolov3/runs/detect/exp20/labels/'
    
    tri_size = opt.trigger_size

    TRIGGER = cv2.resize(cv2.imread(f"../source/trigger_patterns/{opt.trigger_pattern}"), tri_size)
    ANNOTATION_PATHS = '../datasets/bdd100k/Annotations/val/'
    output_path = '../datasets/coco/poison/test_poison_img_oda_bdd100k_yolov3/'
    TP_ANNOTATION_PATHS = '../datasets/coco/poison/tp_annotations_oda_bdd100k_yolov3/'
    ...
    print('dealing with annotations: ')
    for each_filename in (test_data):
        each_filename = each_filename[:-4]
        try:
        ...
        for gt_obj in gt_box:
            for i, pred_box in enumerate(pred_box_yolo):
                ...
                if gt_obj[0] == 'pedestrian' or gt_obj[0] =='rider':
                    gt_obj[0] = 'person'
